Module Module_TIMESHEET
    ...
    Sub UpdateEmployees(ByRef ErrorList() As String, Optional ByVal DeleteAllEmployeesFirst As Boolean = True)
        'UPDATE EMPLOYEES:
        ...
        If DeleteAllEmployeesFirst Then
            Module_DanG_MySQL_Tools.DeleteMyRecord("tblEmployees", frmMainGIForm.myConnString, "", ErrMessage)
        End If
        EmployeesArr = MySQLToArray(frmMainGIForm.myConnString, "SELECT * FROM tblEmployees", ErrMessage, 3, Dic_Employees)
        Percentage = 0.0F
        ...
        For RowIDX = 2 To TotalRows
            ...
            UpdateNeeded = False
            Extract = ExtractArr(RowIDX, 1) 'From Spreadsheet NAME = EMPNO
            ...
            FieldValues = Chr(34) & Firstname & Chr(34)
            FieldValues = FieldValues & "," & Chr(34) & Lastname & Chr(34)
            FieldValues = FieldValues & "," & Chr(34) & EmpNo & Chr(34)
            FieldValues = FieldValues & "," & Chr(34) & description & Chr(34)
            FieldValues = FieldValues & "," & Chr(34) & LastPWChange & Chr(34)
            FieldValues = FieldValues & "," & Chr(34) & LastUpdated & Chr(34)
            FieldValues = FieldValues & "," & Chr(34) & Empno_7Digits & Chr(34)

            FullName = Firstname & " " & Lastname
            If FoundName Then 'EmpNo already in DB:
    ...
    Sub ImportData(ByRef ErrorList() As String, ByVal ImportDate As String, Optional ByVal LastImportDate As String = "")
        ...
        InboundFieldnames = InboundFieldnames & "," & DeliveryInfoFieldsArr(30) 'Supplier Code
        TotalFields = UBound(ExtractArr, 2)
        ...
        For RowIDX = 2 To TotalRows
            ...
            DeliveryInfo_FieldValues = DeliveryInfo_FieldValues & "," & Chr(34) & strSupplierCode & Chr(34) '3
    ...
    Function Get_TotalHours(ScrollControlFrame As ScrollableControl, strDeliveryDate As String, strDeliveryRef As String,
                            ...
                            Optional DelimChar As String = "|") As Double
        Dim ctrl As Control
                ...
                If UCase(ControlName) = UCase("txtOperativeFinishTime:") & CStr(OpIDX) Then
                    ...
                    If dtEndTime >= dtStartTime Then
                        ...
                        Else
                            strDays = ""
                        End If
                        strTime = strDays & strHours & ":" & strMins & ":" & strSecs
                        ...
                        Call frmMainGIForm.InsertValueIntoForm("GI_TIMESHEET", strComments, strTime)
        ...
        Call frmMainGIForm.InsertValueIntoForm("GI_TIMESHEET", "txtTotalHours", FinalTime)
        ...
        If dblStartTime > 0 And dblEndTime > 0 Then
            If dtEndDateTime > dtStartDateTime Then
                ...
                If RoundUpToNearest15Mins Then
                    ...
                    End If
                End If
                strTotalHours = intHours.ToString
                strTotalMinutes = intMins.ToString
                strTotalSeconds = intSecs.ToString
            Else
    ...
    Function Get_DropdownItems(ConnString As String, strSQL As String, Criteria As String, Messages As String) As Object
        ...
        If Not IsNothing(ItemArr) And UBound(ItemArr) > 0 Then
    ...
    Function InsertOperatives(AddNewRow As Boolean, strDeliveryDate As String, strDeliveryRef As String, strASNNo As String, OpFrameControl As ScrollableControl,
                         ...
                         Optional ByVal DisableFinishTimeEntry As Boolean = True) As Long
    ...
    Function Get_Reference_Lock_Info(strDeliveryDate As String, strDeliveryRef As String, ByRef STATUS As String,
        ...
        Dim ErrMessage As String = ""
        Dim TableName As String = "tblReferenceLock"
        Dim RecID As String
        ...
        LoadedOK = Find_myQuery(frmMainGIForm.myConnString, TableName, SearchField, SearchText, FieldType, ReturnField, RecID, AllValues,
                                   AllFields, SearchCriteria, SortFields, Reversed, ErrMessage)

        If LoadedOK Then
            'Someone is EDITING THIS REFERENCE ALREADY !
            Get_Reference_Lock_Info = False
        ...
        Else
            'OK CLEAR TO EDIT THIS REFERENCE:
    ...
    Sub DeletePartsRow(strDeliveryDate As String, strDeliveryRef As String, strCheckFieldName As String,
        ...
        Catch ex As Exception
            ...
            frmMainGIForm.logger.LogError("GI_Error_v1_1.log", Application.StartupPath, Message, "TestLogin()", frmMainGIForm.GetComputerName() & "," & frmMainGIForm.GetIPv4Address() & "," & frmMainGIForm.GetIPv6Address() & ", OPERATOR Logged in:" & frmMainGIForm.myUsername)
    ...
    Function Get_TotalHours_From_DB(strDeliveryDate As String, strDeliveryRef As String) As Double
        ...
        If LoadedOK Then
            ...
            If TotalHoursSPAN.Days > 0 Then
                strFinalTotalHours = CStr(TotalHoursSPAN.Days) & "d"
            End If
            ...
            If FoundCtrl IsNot Nothing Then
                ...
                frmMainGIForm.InsertValueIntoForm("GI_TIMESHEET", FoundCtrl.Name, strFinalTotalHours)
            End If
        End If
    ...
    Sub PopulateControls(TheDeliveryDate As String, TheDeliveryRef As String, Optional ASNNo As String = "")
        ...
        If Len(ASNNo) > 0 Then
            ...
            'comASNNo
            frmMainGIForm.InsertValueIntoForm("GI_TIMESHEET", "comASNNo", ControlASN)
        ...
        Beep()
        Clear_Entry_Controls("GI_TIMESHEET", 1, 40)
        Clear_Entry_Controls("GI_TIMESHEET", 801, 807)
        SearchText = strDeliveryRef
        SearchField = "DeliveryReference"
        FieldType = "STRING"
        Reversed = False
        ...
        If LoadedOK(3) Then
            ...
            Do While FieldIDX < 6
                ...
                End If
                'FoundCtrl = frmMainGIForm.FindControl_Recursive(frmMainGIForm.)
                FoundCtrl = frmMainGIForm.FindControls("GI_TIMESHEET", "", CStr(NewTAGNumber), ChildCtrl)
                If FoundCtrl IsNot Nothing Then
                    'put value into control:
                    frmMainGIForm.InsertValueIntoForm("GI_TIMESHEET", FoundCtrl.Name, strResult)
                    'Insert into Dic_Controls:
        ...
        Do While ControlRowNumber <= OpTotalRows
            ...
            If LoadedOK(4) Then
                ...
                frmMainGIForm.logger.LogError("OpProperties", Application.StartupPath, ErrMessage, "PopulateControls()", frmMainGIForm.GetComputerName() & "," & frmMainGIForm.GetIPv4Address() & "," & frmMainGIForm.GetIPv6Address() & ", OPERATOR LOGGED IN:" & frmMainGIForm.myUsername)
        ...
        If LoadedOK(5) Then
            ...
            Do While ControlRowNumber <= ShortTotalRows
                ...
                If LoadedOK(5) Then
                    ...
                    frmMainGIForm.InsertValueIntoForm("GI_TIMESHEET", "txtTotalShorts", CStr(ControlRowNumber))
        ...
        If LoadedOK(6) Then
            ...
            Do While ControlRowNumber <= ExtraTotalRows
                ...
                If LoadedOK(6) Then
                    ...
                    frmMainGIForm.InsertValueIntoForm("GI_TIMESHEET", "txtTotalExtras", CStr(ControlRowNumber))
    ...
    Sub ResetControls(Optional LowerRange As Long = 0, Optional UpperRange As Long = 0, Optional Confirmation As Boolean = True,
                      Optional FrameName As String = "", Optional ClearFrameControls As Boolean = False)
        Dim TagNumber As Long
        Dim IDX As Long
        ...
        If Len(FrameName) > 0 Then
            ctrlFrame = GetFrameControl("GI_TIMESHEET", FrameName)
        ...
        If Answer = vbYes Then
            For IDX = LowerRange To UpperRange
                TagNumber = IDX
                FoundControl = frmMainGIForm.FindControls("GI_TIMESHEET", "", CStr(IDX))
                If FoundControl IsNot Nothing Then
                    frmMainGIForm.InsertValueIntoForm("GI_TIMESHEET", FoundControl.Name, "")
                End If
            Next
        End If
                ...
                If UCase(ControlDeliveryRef) = UCase(DeliveryRef) And UCase(ctrlProperty.ControlType) = "COMBOBOX" Or UCase(ControlDeliveryRef) = UCase(DeliveryRef) And UCase(ctrlProperty.ControlType) = "TEXTBOX" Then
                    ...
                    If UCase(ControlDBTable) = UCase(DBTables(1)) Then
                        ...
                        If Len(Fieldnames(1)) = 0 Then
                            Fieldnames(1) = "Total_Operatives,UpdatedByUsername,UpdatedbyEmpNo,UpdatedByName,DeliveryDate"
                            FieldValues(1) = OpTotalRows & "," & strUsername & "," & strEmpNo & "," & strName & "," & ControlValue
                        Else
    ...
    Function Get_RecordIDs(strDeliveryDate As String, strDeliveryRef As String, LowerTagNumber As Long, TableName As String, strFrameControl As String,
                           ...
                           Optional Dic_UpdateCriteria As Object = Nothing, Optional ByVal ExtraCriteria As String = "") As Long
        Dim ctrlProperty As clsControls
        Dim RowIDX As Long
        ...
        Else
            ControlLowerTAG = ctrlProperty.ControlStartTAG
            ControlUpperTAG = ctrlProperty.ControlEndTAG
            FrameControl = GetFrameControl("GI_TIMESHEET", strFrameControl)
    ...
    Function Save_TotalHours(strDeliveryDAte As String, strDeliveryRef As String, ByRef TimesheetHrs As Object) As Boolean
        ...
        FLMFrame = GetFrameControl("GI_TIMESHEET", "Frame_FLMDetails")
        OpFrame = GetFrameControl("GI_TIMESHEET", "Frame_Operatives")
        ...
        FoundLabourHrsRecord = Find_myQuery(frmMainGIForm.myConnString, TableName, SearchField, SearchValue, "STRING",
        ...
        If Len(DeliveryInfoRecID) > 0 Then
            NeedsUpdate = True
        Else
            NeedsUpdate = False
        End If
        'tblLabourHours DB TABLE:
        FieldNames = "Total_FLM_Hours,Total_Labour_Hours"
        FieldValues = dblFLMHours & "," & dblTotalOpHours
        If FoundLabourHrsRecord = True Then
            LabHoursSaveOK = InsertUpdateRecords_Using_Parameters(frmMainGIForm.myConnString, NeedsUpdate, "", TableName, FieldNames, FieldValues,
                    ...
                    If UCase(ControlDBTable) = UCase(DBTAbles(4)) Then
                        ...
                        If UCase(ControlFieldname) = "OP_STARTTIME" Then
                            ...
                            Else
                                ...
                                frmMainGIForm.logger.LogError("GI_ERRORS_1_0.log", Application.StartupPath, ErrMessage,
    ...
    Sub ADD_ROW_TO_DATA_GRID_VIEW_OPERATIVES(dgv As Object, RowNum As Long)
        Dim IDX As Int32
