...
Public Class frmCreateUsers
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        ...
        UpdateNames("tblEmployees")


    End Sub

    Sub UpdateNames(Optional ByVal PassedTableName As String = "", Optional ByVal ConfirmChanges As Boolean = False)
        ...
        TotalRows = 0
        TotalFields = 0
        If Len(PassedTableName) > 0 Then
            DBUsersTAble = PassedTableName
        Else
        ...
        Do While RowIDX <= TotalRows
            ...
            If SpacePos > 0 Then
                ...
                strUsername = Mid(SheetFirstname, 1, 1) & SheetLastname
            Else
                    ...
                    If SpacePos > 0 Then 'The Name - 2nd Col - has a space
                        ...
                        Else
                            ...
                            If FirstAnswer = vbCancel Then
                                ReallyQuit = MsgBox("Do you really want to quit this update ?", vbYesNo, "Quit Update ? ")
            ...
            Else
                '**************************************************
                'CANNOT FIND THE EMPLOYEE NUMBER in DATABASE: - does this mean the user wants the name in the table ???
                'ALSO - if the spreadsheet is a definitive list - does this name need deleting from the database ?
                'IS THERE A DUPLICATE NAME ALREADY UNDER ANOTHER EMPLOYEE NUMBER ???
                '**************************************************
                strUsername = ""
                If Len(SheetFirstname) > 0 Then

                    If Len(SheetLastname) > 0 Then
                        strUsername = LCase(Mid(SheetFirstname, 1, 1)) & LCase(SheetLastname)
                    Else
                        strUsername = LCase(SheetFirstname)
                    End If
