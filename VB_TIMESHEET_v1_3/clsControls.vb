Public Class clsControls
    ...
    Private _CtrlOpComments As String
            ...
            Else
                ...
                ElseIf InStr(ucase(txtControl.Name), "OP") > 0 Then
                    ...
                    If testCollection IsNot Nothing Then
                        dic_Controls = testCollection
                    End If
                    'PUT INTO ControlOpSTARTDATETIME PROPERTY:
    ...
    Private Sub txtBoxAfterUpdate_change(sender As Object, e As System.EventArgs) Handles txtBoxAfterUpdate.TextChanged
        ...
        If TypeOf (sender) Is System.Windows.Forms.TextBox Then
            ...
            If UCase(Me.ControlName) = UCase("txtOP_STARTTIME") Then

                If Len(NewValue) > 4 And InStr(NewValue, ":") > 0 Then
                    ...
                    'TimeValue = Me.ControlDeliveryDate & " " & NewValue
                    'Need to be saved for NEXt TAG (FINISH TAG) ALSO so that both dates are in SAME TAG (FINISH TAG) to be able to calculate.
                    TimeValue = Now().ToString("yyyy-MM-dd") & " " & NewValue
                    TimeArr = Split(TimeValue, ":")
                    If IsDate(TimeValue) Then
                        myTEXTBOX.Text = Format(CDate(TimeValue), "HH:mm")
                    End If
                    ChangeProperty = "TheControl"
                    ...
                    testCollection = UpdateCollection(dic_Controls, Nothing, myTEXTBOX, Me.ControlDeliveryDate, Me.ControlASN,
                                                      Me.ControlDeliveryRef, TagNumber, ChangeProperty)
                    ChangeProperty = "OpStartTime"
                    testCollection = UpdateCollection(dic_Controls, Nothing, TimeValue, Me.ControlDeliveryDate, Me.ControlASN,
                                                      Me.ControlDeliveryRef, TagNumber, ChangeProperty)
                    TagNumber = CStr(CInt(TagNumber) + 1)
                    testCollection = UpdateCollection(dic_Controls, Nothing, TimeValue, Me.ControlDeliveryDate, Me.ControlASN,
                                                      Me.ControlDeliveryRef, TagNumber, ChangeProperty)
                    'if anything entered into Time Finish - recalculate total time ?

                End If
            End If
            If UCase(Me.ControlName) = UCase("txtOP_FINISHTIME") Then
                If Len(NewValue) > 4 And InStr(NewValue, ":") > 0 Then
                    'TimeValue = Me.ControlDeliveryDate & " " & NewValue
                    TimeValue = Now().ToString("yyyy-MM-dd") & " " & NewValue
                    TimeArr = Split(TimeValue, ":")
                    ...
                    testCollection = UpdateCollection(dic_Controls, Nothing, TimeValue, Me.ControlDeliveryDate, Me.ControlASN,
                                                      Me.ControlDeliveryRef, TagNumber, ChangeProperty)
                    If IsDate(ControlOpEndDateTime) Then
                        'VET both START and END / FINISH times. Both must be greater than default of 1900-01-01.

                        spanTotalTime = CDate(Me.ControlOpEndDateTime).Subtract(CDate(ControlOpStartDateTime))
                        dblHours = spanTotalTime.TotalHours
                        dblHours = CDbl(FormatNumber(dblHours, 2))
                        'If IsNumeric(NewValue) Then
                        If dblHours > 0 And dblHours < 24 Then
                            dblMins = spanTotalTime.TotalMinutes
                            decMins = CDec(dblMins)
                            decHours = Math.Floor(decMins / 60)
                            decSecs = 0
                            'true_mins = dblmins-(dechours*60)
                            strTotalTime = CStr(decHours) & "h " & CStr(decMins) & "m "


                            'SAVE to OpComments:
                            TagNumber = CStr(CInt(TagNumber) + 1)
                            ChangeProperty = "OpComments"
                            testCollection = UpdateCollection(dic_Controls, Nothing, strTotalTime, Me.ControlDeliveryDate, Me.ControlASN,
                                                      Me.ControlDeliveryRef, TagNumber, ChangeProperty)
                            ChangeProperty = "OpComments"
                            testCollection = UpdateCollection(dic_Controls, Nothing, dblHours, Me.ControlDeliveryDate, Me.ControlASN,
                                                      Me.ControlDeliveryRef, TagNumber, ChangeProperty)
                        Else
                            'HOURS falls outside the allowable range:
                            
                        End If
                        'End If
                    End If
                    'Calculate Total Time and put into correct fields:
                    'Use SPAN and subtract.
                End If
            End If
            'EMPLOYEE NUMBER ENTERED BY USER:
            If UCase(Me.ControlName) = UCase("txtSearchEmployeeNo") Then
            ...
            If UCase(Me.ControlName) = UCase("txtOperationalComments") Then
                ...
                testCollection = UpdateCollection(dic_Controls, Nothing, NewValue, Me.ControlDeliveryDate, Me.ControlASN,
                                                      Me.ControlDeliveryRef, TagNumber, ChangeProperty)
            End If
            If UCase(Me.ControlName) = UCase("txtTotalTime") Then
                FoundControl = frmMainGIForm.FindFrameControls(CPFormName & FormID, Me.ControlName)
                NewValue = FoundControl.Text
                If IsDate(ControlOpEndDateTime) Then
                    spanTotalTime = CDate(Me.ControlOpEndDateTime).Subtract(CDate(ControlOpStartDateTime))
                    NewValue = spanTotalTime.ToString()
                End If
                myTEXTBOX = CType(FoundControl, System.Windows.Forms.TextBox)
                myTEXTBOX.Text = NewValue
                ChangeProperty = "TheControl"
            ...
            End If
            If UCase(Me.ControlName) = UCase("txtTotalHours") Then
                FoundControl = frmMainGIForm.FindFrameControls(CPFormName & FormID, Me.ControlName)
                NewValue = FoundControl.Text
                'call function to loop current records in query - use MysqlToArray() routine
            End If
    ...
    Public Property Control_Opcomments() As String
        Get
            Return _CtrlOpComments
        End Get
        Set(value As String)
            _CtrlOpComments = value
        End Set
    End Property
