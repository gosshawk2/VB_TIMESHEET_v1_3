...
Module DanG_EXCEL_Module
    ...
    Function XL_CSVFileToArray(ByRef NewstrArray As String(,), ByVal CSVFilename As String, Optional ByRef TotalFields As Long = 0,
        ...
        'Me.rtbOutput.Text = ""
        'frmProgressBar.Hide()
        XL_CSVFileToArray = numRowsRead
    End Function


    Function XL_GetFieldValue_From_Fieldname(ByVal AllRows As Object, ByVal RowIDX As Long, ByVal FieldName As String, Optional ByRef FieldPos As Long = 0) As String
        Dim ReturnValue As String
        Dim colIDX As Long
        Dim ArrayValues As String

        'AllRows(Fields,0) - row 0 contains all the fieldnames
        XL_GetFieldValue_From_Fieldname = ""
        ReturnValue = ""
        For colIDX = 0 To UBound(AllRows, 1)
            ...
            'colIDX = colIDX + 1
        Next
        XL_GetFieldValue_From_Fieldname = ReturnValue
    End Function
    ...
    Public Function ExtractExcelRows(
        ...
        Optional ErrMessage As String = "") As Object(,)
            ...
            If Proceed Then
                ...
                Try
                    Try
                        ...
                        xlUsedRange = xlWorkSheet.Range(xlWorkSheet.Range(xlWorkSheet.Cells(StartRow, StartCol).address, xlWorkSheet.Cells(LastRow, LastCol).address).Address)
                        'CHECK xlUsedRange for blank cols:
                        CheckValue = CType(xlUsedRange.Cells(1, 1).value(Excel.XlRangeValueDataType.xlRangeValueDefault), String)
                        If CheckValue Is Nothing Then
                            'No column heading for first column:
                            CheckValue = CType(xlUsedRange.Cells(1, 2).value(Excel.XlRangeValueDataType.xlRangeValueDefault), String)
                            If CheckValue Is Nothing Then
                                MessageBox.Show("second Column appears to be blank - cannot load")
                                Exit Function
                            Else
                        ...
                        End If
                    Catch theException As Exception
                        ErrMessage = theException.ToString
    ...
    Function RC_Ref(FormRow As Integer, FormCol As Integer, Row1 As Integer, Col1 As Integer, Row2 As Integer, Col2 As Integer, AbsRef1 As Integer, AbsRef2 As Integer) As String
        ' ** create a text string for an absolute or relative range reference R1C1:R[2]C[2}
        Dim Temp1 As String, Temp2 As String
        If AbsRef1 = 0 Then
            Temp1 = "R[" & Row1 - FormRow & "]C[" & Col1 - FormCol & "]"
        Else
            Temp1 = "R" & Row1 & "C" & Col1
        End If
        If AbsRef2 = 0 Then
            Temp2 = "R[" & Row2 - FormRow & "]C[" & Col2 - FormCol & "]"
        Else
            Temp2 = "R" & Row2 & "C" & Col2
        End If
    ...
    Sub ExcelOutput_From_Array(ByVal xlFileFormat As Integer, ByVal OutputFilename As String, ByVal SheetOutputArr(,) As String,
                               Optional ArrIndexStartRow As Integer = 0,
                               Optional ArrIndexStartCol As Integer = 0,
                               Optional SheetRowStart As Integer = 1,
                               Optional SheetColStart As Integer = 1
                               )
        'SETUP EXCEL OBJECT:
        Dim Proceed As Boolean = False
        'Dim xlsCells As Excel.Range = Nothing
        Dim ExcelArray(,) As Object
        Dim dtFoundDate As Date
        Dim xlsWorkBook As Microsoft.Office.Interop.Excel.Workbook
        Dim xlsWorkSheet As Microsoft.Office.Interop.Excel.Worksheet
        ...
        Dim resourcesFolder = IO.Path.GetFullPath(Application.StartupPath & "\..\..\Resources\")
        Dim WBFileName As String = OutputFilename 'PATH NAME is implied 
        Dim SheetIDX As Integer
        Dim SheetName As String = ""
        Dim TotalSheets As Integer
        ...
        Dim LastSearchDate_FirstRow As Long = 0
        Dim LastSearchDate_LastRow As Long = 0
        Dim xlUsedRange As Microsoft.Office.Interop.Excel.Range
        Dim ArrRowIDX As Integer
        Dim ArrColIDX As Integer
        Dim SheetRowIDX As Integer
        Dim SheetColIDX As Integer
        Dim TotalFields As Integer
        Dim Percentage As Single = 0.0F
        Dim FoundText As String = ""
        Dim Titles() As String = Nothing
        ...
        frmMainGIForm.pbarMain.Style = ProgressBarStyle.Blocks
        frmMainGIForm.pbarMain.Value = 0
        frmMainGIForm.txtMessages.Text = "Please Wait ... Plot To Spreadsheet"
        ExcelArray = Nothing
        ArrRowIDX = ArrIndexStartRow
        ArrColIDX = ArrIndexStartCol
        SheetRowIDX = SheetRowStart
        SheetColIDX = SheetColStart
        If Len(OutputFilename) > 0 Then
            TheFilename = OutputFilename
        Else
            TheFilename = "RegisterOutput.xlsx"
        End If
        'ExcelArray = SheetOutputArr
        LastRow = UBound(SheetOutputArr, 1)
        LastCol = UBound(SheetOutputArr, 2)
        ...
        If SheetExists("", "sheet1", xlsWorkBook) Then
            xlsWorkSheet = CType(xlsWorkBook.Sheets.Item("sheet1"), Microsoft.Office.Interop.Excel.Worksheet)
            xlsWorkSheet.Name = "Output_" & Now().ToString("ddMMyyyy HHmm")
        Else
            xlsWorkSheet = DirectCast(xlsapp.Worksheets.Add(xlsWorkBook.Worksheets(1), Type.Missing, Type.Missing, Type.Missing), Excel.Worksheet)
            xlsWorkSheet.Name = "Output_" & Now().ToString("ddMMyyyy HHmm")
        End If

        'Fetch the data from the array and plot onto new sheet:
        ArrRowIDX = ArrIndexStartRow
        SheetRowIDX = SheetRowStart

        xlUsedRange = xlsWorkSheet.Range(xlsWorkSheet.Cells(SheetRowStart, SheetColStart), xlsWorkSheet.Cells(LastRow, LastCol))
        'sort sheet before extraction:

        Try

            ExcelArray = CType(xlUsedRange.Value(Excel.XlRangeValueDataType.xlRangeValueDefault), Object(,))
            'CType(xlUsedRange.Value(Excel.XlRangeValueDataType.xlRangeValueDefault), Object(,)) = SheetOutputArr
            'MsgBox("Row2 col1: BEFORE allocation= " & ExcelArray(2, 1))
            If SheetOutputArr IsNot Nothing Then
                Dim bounds As Integer = SheetOutputArr.GetUpperBound(0)
                'dtDeliveryDate = ExcelArray
                TotalFields = UBound(SheetOutputArr, 2)

                Do While SheetRowIDX < LastRow
                    SheetColIDX = SheetColStart
                    ArrColIDX = ArrIndexStartCol
                    Do While SheetColIDX < LastCol
                        'xlsWorkSheet.Range(xlsWorkSheet.Cells(1, 1), xlsWorkSheet.Cells(LastRow, LastCol)) = SheetOutputArr(ArrRowIDX, ArrColIDX)
                        ExcelArray(SheetRowIDX, SheetColIDX) = SheetOutputArr(ArrRowIDX, ArrColIDX)
                        SheetColIDX = SheetColIDX + 1
                        ArrColIDX = ArrColIDX + 1
                    Loop
                    Percentage = (SheetRowIDX / LastRow) * 100
                    frmMainGIForm.pbarMain.Value = CInt(Percentage)
                    Application.DoEvents()
                    SheetRowIDX = SheetRowIDX + 1
                    ArrRowIDX = ArrRowIDX + 1
                Loop
                xlUsedRange.Value(Excel.XlRangeValueDataType.xlRangeValueDefault) = ExcelArray
        ...
        'xlUsedRange = xlsWorkSheet.Range(xlsWorkSheet.Cells(1, 1), xlsWorkSheet.Cells(LastRow, LastCol))
        'xlWorkSheet.Range(xlWorkSheet.Cells(RowIDX, SearchCol), xlWorkSheet.Cells(RowIDX, SearchCol)) = xlUsedRange
        'ExcelArray = CType(xlUsedRange.Value(Excel.XlRangeValueDataType.xlRangeValueDefault), Object(,))
        'ExcelArray = SheetOutputArr
        'CType(xlUsedRange.Value(Excel.XlRangeValueDataType.xlRangeValueDefault), Object(,)) = ExcelArray
        'xlsWorkSheet.Range(xlsWorkSheet.Cells(1, 1), xlsWorkSheet.Cells(LastRow, LastCol)) = ExcelArray
        frmMainGIForm.txtMessages.Text = "Please Wait ... Processing Spreadsheet Data"
        If InStr(TheFilename, "xlsx") > 0 Then
            xlFileFormat = 51
        End If
        xlsWorkBook.SaveAs(TheFilename, xlFileFormat)
        Message = "New Workbook: saved as: " & TheFilename
        ...
        'releaseObject(xlsCells)
        'releaseObject(xlUsedRange)
        'releaseObject(xlsWorkSheets)
        'releaseObject(xlsWorkSheet)
        'releaseObject(xlsWorkBook)
        'releaseObject(xlsWorkBooks)
        'releaseObject(xlsApp)
    ...
    Sub SearchTemplate(ByVal WholeSheet(,) As Object, ByVal SearchField As String, ByRef FoundPos As String, Optional ByVal StartRow As Long = 1, Optional ByVal StartCol As Long = 1, Optional ByVal MaxRows As Long = 0, Optional ByVal MaxColumns As Long = 0)
        Dim RowIDX As Long
        Dim ColIDX As Long
        ...
        If MaxRows > 0 Then
            LastRow = MaxRows
        Else
            LastRow = UBound(WholeSheet, 1) + 1
        End If
        If MaxColumns > 0 Then
            LastCol = MaxColumns
        Else
            LastCol = UBound(WholeSheet, 2) + 1
        End If
        RowIDX = StartRow
        Do While RowIDX < LastRow - 1
            For ColIDX = StartCol To LastCol - 1
                If UCase(WholeSheet(RowIDX, ColIDX) = UCase(SearchField)) Then
                    'FOUND THE FIELD !
                    FoundRow = RowIDX
                    FoundColumn = ColIDX
                    Exit Do
        ...
        FoundPos = CStr(FoundRow) & "," & CStr(FoundColumn)

    End Sub

    Sub Insert_Data_Into_Sheet(ByVal OriginalWorkbook As String, ByVal NewWorkbookFilename As String, ByVal Dic_Data As Object,
                               Optional ByVal SaveFileFormat As Integer = 51,
                               Optional ByVal ArrIndexStartRow As Integer = 0,
                               Optional ByVal ArrIndexStartCol As Integer = 0,
                               Optional ByVal SheetRowStart As Integer = 1,
                               Optional ByVal SheetColStart As Integer = 1,
                               Optional ByVal EndRow As Integer = 0,
                               Optional ByVal EndCol As Integer = 0)
        Dim PlotArray(,) As Object
        Dim xlsWorkBook As Microsoft.Office.Interop.Excel.Workbook
        Dim xlsWorkSheet As Microsoft.Office.Interop.Excel.Worksheet
        Dim xlsWorkSheets As Microsoft.Office.Interop.Excel.Worksheets
        Dim xlsAPP As New Microsoft.Office.Interop.Excel.Application
        Dim resourcesFolder = IO.Path.GetFullPath(Application.StartupPath & "\..\..\Resources\")
        Dim WBFileName As String = NewWorkbookFilename 'PATH NAME is implied
        Dim OriginalWBPath As String
        Dim SheetIDX As Integer
        Dim SheetName As String = ""
        Dim TotalSheets As Integer
        ...
        Dim xlFileFormat As Integer
        Dim HashItem As String
        ...
        If Len(OriginalWorkbook) = 0 Then
            'CREATE NEW SHEET:
            xlsWorkBook = xlsAPP.Workbooks.Add()
        Else
            'OPEN EXCEL WORKBOOK:
            OriginalWBPath = ""
            'OpenWorkbookAndSetGlobal(OriginalWorkbook, OriginalWBPath, True, ErrMessage, True)
            xlsWorkBook = xlsAPP.Workbooks.Open(OriginalWBPath & OriginalWorkbook)
        End If
        xlFileFormat = SaveFileFormat
        xlsAPP.Visible = True
        Percentage = 0.0F
        xlUsedRange = Nothing
        frmMainGIForm.pbarMain.Style = ProgressBarStyle.Blocks
        ...
        SheetRowIDX = SheetRowStart
        SheetColIDX = SheetColStart
        'If Len(WBFileName) > 0 Then
        'TheFilename = WBFileName
        'Else
        'TheFilename = "Output.xlsx"
        'End If
        'ExcelArray = SheetOutputArr
        'LastRow = UBound(InsertData, 1)
        'LastCol = UBound(InsertData, 2)
        ...
        ArrRowIDX = ArrIndexStartRow
        SheetRowIDX = SheetRowStart
        LastCol = xlsWorkSheet.Cells(1, xlsWorkSheet.Columns.Count).end(Excel.XlDirection.xlToLeft).column + 1
        LastRow = xlsWorkSheet.Cells(xlsWorkSheet.Rows.Count, 1).End(Excel.XlDirection.xlUp).row + 1
        ...
        Try
            ...
            PlotArray = CType(xlUsedRange.Value(Excel.XlRangeValueDataType.xlRangeValueDefault), Object(,))
            'CType(xlUsedRange.Value(Excel.XlRangeValueDataType.xlRangeValueDefault), Object(,)) = SheetOutputArr
            'MsgBox("Row2 col1: BEFORE allocation= " & ExcelArray(2, 1))
            FoundPos = "0,0"
            For Each varKEY In Dic_Data.keys
                HashItem = Dic_Data.item(varKEY)
                DanG_EXCEL_Module.SearchTemplate(PlotArray, varKEY, FoundPos)
                FoundArr = Split(FoundPos, ",")
                FoundRow = CInt(FoundArr(0))
                FoundCol = CInt(FoundArr(1))

                If FoundRow > 0 And FoundCol > 0 Then
                    PlotArray(FoundRow, FoundCol) = HashItem
                End If
            Next
            'If InsertData IsNot Nothing Then
            'Dim bounds As Integer = InsertData.GetUpperBound(0)
            'dtDeliveryDate = ExcelArray
            'TotalFields = UBound(InsertData, 2)


            xlUsedRange.Value(Excel.XlRangeValueDataType.xlRangeValueDefault) = PlotArray
        ...
        MainMessage = "New Workbook: saved as: " & TheFilename
        frmMainGIForm.txtMessages.Text = MainMessage
    ...
    Sub SortSheet(WBFileName As String, Worksheetname As String, ByVal StartRow As Long, ByVal StartCol As Long, SortColumn1 As Long, ByRef ExcelArray(,) As Object,
        ...
        If IO.File.Exists(WBFileName) Then
            ...
            If SortColumn1 > 0 Then
                ...
                StartRow = 2
                'for DELIVERY DATE:
                'The following may give an error ????? may needed to add .address after brackets ???
